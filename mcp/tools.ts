/**
 * Inherent OS — MCP tool definitions, shared between the stdio transport
 * (mcp/server.ts, for Claude Code / Codex running locally) and the HTTP
 * transport (src/app/api/mcp/route.ts, for the Claude.ai and ChatGPT custom
 * connectors, which run in the cloud and need a URL instead of a subprocess).
 *
...
export function registerTools(server: McpServer, db: PrismaClient) {
  const MODELS = {
    Account: db.account,
    Contact: db.contact,
    Deal: db.deal,
